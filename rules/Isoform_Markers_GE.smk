...
rule isoform_marker_ge:
    input:
	    input_rda_file = expand("{start}/{sample_name_ge}{end}",zip,start=COMPLEMENT_ISOFORM_INPUT_RDA_LIST_START,sample_name_ge=ISOFORM_SAMPLE_NAME_GE_RAW,end=COMPLEMENT_ISOFORM_INPUT_RDA_LIST_END)
    output:
	    output_rda_file = expand("{start}/{sample_name_ge}{end}",zip,start=COMPLEMENT_ISOFORM_OUTPUT_RDA_LIST_START,sample_name_ge=ISOFORM_SAMPLE_NAME_GE_RAW,end=COMPLEMENT_ISOFORM_OUTPUT_RDA_LIST_END)
    params:
	    ...
	    isoform_mtx = expand("{folder_path}{sample_name_ge}/{sample_name_ge}/transcript_processed_feature_bc_matrix/",zip,folder_path=ISFORM_MTX_INTPUT,sample_name_ge=ISOFORM_SAMPLE_NAME_GE_RAW),
	    gtf_ge = GTF_GE,
