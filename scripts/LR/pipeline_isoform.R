...
rownames(scmat) <- merge_isoform_df$transcript_ensembl
...
for(i_gene in unique(isoswitch_df$external_gene_name)){
	...
	data <- sobj@assays$Isoform@data[unique(tmp_df_isoform$ensembl_transcript_id_version),] %>% t() %>%as.data.frame()

	colnames(data) <- paste0(colnames(data)[1:length(colnames(data))],".",i_gene)
	...
	vec_gene <- unique(isoswitch_df[isoswitch_df$external_gene_name == i_gene,]$ensembl_transcript_id_version)
	
	
	i_gene_annotation_from_gtf <- gtf %>% 
	  dplyr::filter(
	    !is.na(gene_name), 
	    gene_name == i_gene
	  )
	 ...
	 if(dim(i_gene_annotation_from_gtf)[1] == 0){i_gene_annotation_from_gtf <- gtf %>% dplyr::filter(transcript_id %in% vec_gene)}
	...
	i_exons <- i_gene_annotation_from_gtf %>% dplyr::filter(type == "exon") %>% dplyr::filter(transcript_id %in% vec_gene)
	...
	p_ggtranscript <-  ggplot(transcript_data_exons,aes(xstart = start,xend = end,y = transcript_id_gene_name)) +
